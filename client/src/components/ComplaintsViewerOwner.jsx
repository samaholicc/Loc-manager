...
function ComplaintsViewerOwner(props) {
  ...
  const getComplaints = async () => {
    try {
      ...
      console.error(err);
      setError(err.message || "Erreur lors de la récupération des plaintes.");
      toast.error(err.message || "Erreur lors de la récupération des plaintes.");
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      >
        <div className="p-6">
          ...
          {loading ? (
            <div className="text-center py-10 text-lg text-gray-600 dark:text-gray-300">Chargement...</div>
          ) : error ? (
            <div className="text-center text-red-500 dark:text-red-400 text-lg font-medium p-5 bg-white dark:bg-gray-800">
              <p>{error}</p>
              <button
                ...
                >
                  ...
                  <div className="mx-3 flex justify-center items-center text-red-500">
                    <MdDeleteForever
                      ...
                      onClick={() => deleteComplaints(ele.room_no)}
                    />
                  </div>
                </div>
              )
